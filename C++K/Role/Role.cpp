...
bool Role::checkObjectCollision(Role& oppo, const bool& isOriginBound)
{
	...
	//��ײǰ��λ��,ע��ֵ�����ú�tileƽ�⣬���ܿ�ס
	objectIntersection = Fand::getIntersectRact(getCollisionBound(insetObject), bound);//2.5f
	//û�з�����ײ
...
void Role::checkLine(const short& tag, const Vec2 & start, const Vec2 & end)
{
	...
	auto collided = Fand::getLineIntersectRact(start, end, bound);
...
void Role::checkTileCollision()
{
	...
	//���һ��tile�Լ���б��һ��Ҫ��
	for (int index = 0; index < BandRact; index++)
	{
		//ע��lua�±� 1 
		//�����0����ָ����tile ����Դ���
		...
		//�����������Ϊ�ƶ���Ƭ��ԭʼλ����Ƭ��������ײ
		if (!boundRacts[index].tileName.empty())
			continue;

		auto bound = getCollisionBound(insetTile);

		Ract intersection = Fand::getIntersectRact(bound, boundRacts[index]);
		if (intersection.origin == Vec2::ZERO)
			continue;

		//б��   
		if (checkTileSlopes(index, boundRacts))
		{
			gotoDesirePosition();
			return;
		}
		...
		{
			if (boundRacts[index].gid > 0 && bound.getMaxY() >= boundRacts[index].origin.y)
			{
...
bool Role::checkTileSlopes(const unsigned short& positionIndex, BoundRact  boundRacts[BandRact])
{
	...
	else if (tileProperty.slopeTowards == SlopeTowards::faceLeft && tileProperty.slopeDegree == SlopeDegree::degree22_5)
	{
		if (getPositionX() >= slopeRact.getMidX())
			if ((boundRacts[BoundPosition::bottomRight].gid != slopeRact.gid && boundRacts[BoundPosition::bottomRight].gid > 0) || (boundRacts[BoundPosition::bottom].gid != slopeRact.gid && boundRacts[BoundPosition::bottom].gid > 0))
				return false;
		// 	m = (y2 - y1) / (x2 - x1);
		float m = (slopeRact.getMaxY() - slopeRact.origin.y) / (slopeRact.getMaxX() - slopeRact.origin.x);
	...
	else if (tileProperty.slopeTowards == SlopeTowards::faceRight && tileProperty.slopeDegree == SlopeDegree::degree22_5)
	{
		...
		float m = (slopeRact.origin.y - slopeRact.getMaxY()) / (slopeRact.origin.x - slopeRact.getMaxX());
		float x = desiredPosition.x *m;
		float interception = getMap()->getTileSize().height / 2;
		float degree = interception / getMap()->getTileSize().height;
		float initSlopeY = slopeRact.getProperty().initSlopeY ? interception : 0.f; //22�Ƚǵĵڶ���tile
...
Ract Role::getCollisionBound(const Vec2& inset, const bool& isOriginBound)
{
	...
	Ract originBox = isOriginBound ? originBound : getBoundingBox();

	auto collisionBox = Fand::RactInset(originBox, inset.x, inset.y);
	auto diff = desiredPosition - this->getPosition(); //��ҵ�ǰ������Ŀ�ĵصĲ��
	auto boundingBox = Fand::RactOffset(collisionBox, diff.x, diff.y); //������������ײ��,����Χ��x,y�᷽�����ƶ�diff.x, diff.y����λ
