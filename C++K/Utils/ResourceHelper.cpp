...
//	return starCount;
//}
void Resh::preloadFrame(const std::string& filename, const Ract& rect)
{
	...
	for (int y = 0; y < rect.origin.y; y++)
	{
		...
		{
			sprintf(name, "%d", index++);
			auto frame = SpriteFrame::createWithTexture(texture, Ract(x * rect.size.width, y * rect.size.height, rect.size.width, rect.size.height));
